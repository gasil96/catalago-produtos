...
public class ProductValidatorRestApplicationTests {  
 ...
 @Order(1)  
  ...
  /**  
 * Body return contain ID, but the send not * */  productDTO.setId("1");  
 ...
 @Order(4)  
  ...
  /**  
 * File 'data.sql' in path resource contains 10 records + 2 add in before tests * */  Assertions.assertEquals(results.getStatusCode(), HttpStatus.OK);  
 ...
 * File 'data.sql' in path resource contains 2 records with "@" in name or description" * */  
  ...
  /**  
 * Valid is paremeter min e max price * */  Assertions.assertEquals(resulsWithParameter.getStatusCode(), HttpStatus.OK);  
...
## Rodando à aplicação
A aplicação esta setada para subir na porta 9999, após baixar o projeto rode 
